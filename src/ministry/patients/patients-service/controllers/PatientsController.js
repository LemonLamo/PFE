...
class PatientsController{
  ...
  async selectHistorique(req, res) {
    const { NIN } = req.params;
    const consultations = (await axios.get(`http://consultations-service/api/consultations?patient=${NIN}`, {headers:{'Authorization': req.headers.authorization}})).data;
    const hospitalisations = (await axios.get(`http://hospitalisations-service/api/hospitalisations?patient=${NIN}`, {headers:{'Authorization': req.headers.authorization}})).data;
    const interventions = (await axios.get(`http://interventions-service/api/interventions?patient=${NIN}`, {headers:{'Authorization': req.headers.authorization}})).data;
