...
class PatientsModel {
  ...
  async UpdateTravail(NIN,code_handicap){
	try{
	await db.execute("UPDATE handicaps SET `travail`=1 WHERE `id`=? AND `code_handicap`=?",[NIN,code_handicap]);
	} catch(err) {
  ...
  async UpdateSolidarity(NIN,code_handicap){
	try{
	await db.execute("UPDATE handicaps SET `solidarity`=1 WHERE `id`=? AND `code_handicap`=?",[NIN,code_handicap]);
	} catch(err) {
		logger.error("ERROR updating travail:",err);
		throw error;
	} 
