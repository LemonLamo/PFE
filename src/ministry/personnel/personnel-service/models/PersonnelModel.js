...
class PersonnelModel {
  ...
  async selectByNINs(NINs) {
    try {
      const [results] = await db.query(
        "SELECT NIN, nom, prenom, sexe, date_de_naissance, lieu_de_naissance, email, telephone, specialite, hopital, service FROM `personnel` WHERE `NIN` IN (?)",
        [NINs]
      );
      return results;
    } catch (error) {
  ...
  async selectByNIN(NIN) {
    try {
      const [results] = await db.query(
        "SELECT NIN, nom, prenom, sexe, date_de_naissance, lieu_de_naissance, email, telephone, specialite, hopital, service FROM `personnel` WHERE `NIN`= ?",
        [NIN]
      );
      return results[0];
    } catch (error) {
