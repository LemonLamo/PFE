...
class AuthController{
    login = async (req, res) => {
        ...
        if (!user.two_factor_enabled){
            ...
            communication.sendEmail(profile.email, 'TWO_FACTOR_TOKEN', { to: profile.email, two_factor_token: token })
            communication.sendSMS(profile.phoneNumber, 'TWO_FACTOR_TOKEN', { to: profile.phoneNumber, two_factor_token: token })
            return res.status(200).json({ successCode: "login.2fa-code", successMessage: "Please enter 2FA code" });
        }
    }
    ...
    activate = async(req, res) => {
        ...
        if (!user)
            return res.status(400).json({ errorCode: "unauthorized.no-user", errorMessage: "No account is associated with this NIN."});

        if (!user.verify_token || !await bcrypt.compare(verify_token, user.verify_token))
            return res.status(400).json({ errorCode: "unauthorized.missing-auth", errorMessage: "Invalid verification token"});
    ...
    enable_2fa = async(req, res) => {
        ...
        if (!login_match)
            return res.status(400).json({ errorCode: "unauthorized.wrong-user", errorMessage: "Wrong user credentials." })
    ...
    forgot_password = async (req, res) =>{
        ...
        try{
            ...
            await communication.sendEmail(profile.email, 'RESET_PASSWORD', email_data)
            return res.status(200).json({ success: true });
        }catch (err) {
            return res.status(400).json({ errorCode: "database-error", errorMessage: err.code });
        }
    ...
    reset_password = async(req, res) => {
        ...
        if (!user.reset_token || !await bcrypt.compare(reset_token, user.reset_token))
            return res.status(400).json({errorCode: "unauthorized.missing-auth", errorMessage: "Invalid reset token"});
