...
exports.fetchInterventions = async (data) => {
    ...
    const interventions = (await axios.post('http://codifications-service/private/interventions', { codes_interventions })).data;
    const codesMap = new Map(interventions.map((x) => [x.code_intervention, { ...x }]));
