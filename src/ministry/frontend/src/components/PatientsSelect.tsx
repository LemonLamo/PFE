...
function PatientsSelect({ onChange, placeholder = '', className = '', state }: SelectProps) {
        ...
        <Combobox value={selectedPatient} onChange={setSelectedPatient} nullable>
            <div className={`z-10 relative w-full ${className}`}>
                ...
                <Combobox.Button className="w-full">
                    <Combobox.Input displayValue={(o: any) => o && o.nom && `${o.nom} ${o.prenom}`} onChange={(event) => setQuery(event.target.value)} className="pl-9 w-full select cursor-default" placeholder={placeholder} autoComplete="off"/>
