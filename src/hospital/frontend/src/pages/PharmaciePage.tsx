...
function PharmacyPage() {
      ...
      header: "", id: "actions", cell: (info) => {
        ...
        return (
          <div className="flex justify-end gap-2">
            <ViewModal onOpen={() => view_medicament(m)}>
            ...
            <DeleteModal onDelete={() => delete_medicament(m.code)}>
  ...
  function view_medicament(m: Medicament) {
    setSelectedMedicament(m);
  }

  async function update_quantite_medicament_add() {
    const code = selectedMedicament.code;
    try {
      await axios.put(`http://localhost:8080/api/medicaments/${code}`, { code: selectedMedicament.code, quantite: selectedMedicament.quantite})
      query.refetch()
      query2.refetch()
  ...
  async function update_quantite_medicament_substract() {
    const code = selectedMedicament.code;
    try {
      await axios.put(`http://localhost:8080/api/medicaments/${code}`, { code: selectedMedicament.code, quantite: -selectedMedicament.quantite! })
      query.refetch()
  ...
  async function delete_medicament(code: string) {
    try{
      await axios.delete(`http://localhost:8080/api/medicaments/${code}`, )
      query.refetch()
    }catch(err : AxiosError | any){
      if (err.response)
        alert(err.response.data.errorCode + ' - ' + err.response.data.errorMessage)
  ...
  const createModal = (
    <div className="flex gap-2">
      <RemplirModal onAdd={update_quantite_medicament_add} onCancel={() => console.log("Cancelled create")}>
        <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3" id="modal-title">
      ...
      <DepenserModal onSubstract={update_quantite_medicament_substract} onCancel={() => console.log("Cancelled create")}>
        <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3" id="modal-title">
  ...
  return (
    <>
      <Card title="Gestion des médicaments" action={createModal} className="w-full">
