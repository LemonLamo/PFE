...
function MesAdmisPage() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const data = (await axios.get(`${baseURL}/api/hospitalisations/mine`))
        .data;
      return data;
    },
  });
  ...
  const tableDefinition = useMemo(
    () => [
      ...
      {
        header: "Motif d'hospitalisation",
        accessorKey: "motif_hospitalisation",
      },
      {
        header: "Résumé d'hospitalisation",
        accessorKey: "resume_hospitalisation",
      },
      {
        header: "",
        id: "actions",
        cell: (info) => {
          ...
          return (
            <div className="flex justify-end gap-2">
              <Link
                to={`/patients/${a.patient.NIN}`}
                className="w-4 transform text-green-500 hover:text-green-700 hover:scale-110"
              >
                {" "}
  ...
  const [selectedInterventions, setSelectedInterventions] =
    ...
    >
      ...
      >
        ...
        <p className="text-gray-600">
          Remplissez ce formulaire pour plannifier une intervention pour{" "}
        ...
        <div className="grid grid-cols-6 gap-2">
          <label className="font-semibold text-slate-700 text-sm col-span-2">
            {" "}
            Intervention:{" "}
          </label>
          <Select
            className="col-span-4"
            options={dictionnaire_interventions}
            placeholder="Intervention"
            onChange={select_intervention}
            state={{
              key: selectedInterventions.code_intervention,
              value: selectedInterventions.nom!,
            }}
          />
          ...
          <input
            className="primary col-span-4"
            type="datetime-local"
            placeholder="Date"
            value={moment(selectedInterventions.date).format(
              "YYYY-MM-DDTHH:mm"
            )}
            onChange={(e) =>
              setSelectedInterventions({
                ...selectedInterventions,
                date: moment(e.target.value, "YYYY-MM-DDTHH:mm").toDate(),
              })
            }
          ></input>

          <label className="font-semibold text-slate-700 text-sm col-span-2 self-start">
            {" "}
            Remarques:{" "}
          </label>
          <textarea
            ...
            value={selectedInterventions.remarques}
            onChange={(e) =>
              setSelectedInterventions({
                ...selectedInterventions,
      ...
      >
        ...
        <p className="text-gray-600">
          Remplissez ce formulaire pour ajouter une remarque à cette
      ...
      >
        ...
        <div className="grid grid-cols-6 gap-2">
          <label className="font-semibold text-slate-700 text-sm col-span-6">
            {" "}
            Mode de sortie:{" "}
          </label>
