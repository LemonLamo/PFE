...
type Props = {
  isOpen: boolean,
  close: () => void,
  action: (arg0: Antecedent) => Promise<void>
}
...
export default function AjouterAntecedentMedical({isOpen, close, action}: Props) {
    ...
    return (
        createPortal(
        <Modal isOpen={isOpen} icon="fa fa-clock-rotate-left" theme={theme} size="sm:max-w-2xl">
            <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3"> Ajouter un antécédent médical</h3>
            <p className="text-gray-600"> Remplissez ce formulaire pour ajouter une antécédent médical.</p>
            <form onReset={reset}>
