...
type Props = {
  isOpen: boolean,
  close: () => void
  action: (arg0: any) => Promise<void>
}

const theme = "primary"

export default function AjouterHandicap({isOpen, close, action}: Props) {
    ...
    return (
        ...
        <Modal isOpen={isOpen} icon="fa fa-person-dots-from-line" theme={theme} size="sm:max-w-2xl">
            <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3"> Ajouter une handicap</h3>
            <p className="text-gray-600"> Remplissez ce formulaire pour ajouter une handicap.</p>
            <div className="grid grid-cols-6 gap-2">
                <label className="font-semibold text-slate-700 text-sm col-span-2"> Handicap<span className="text-red-500">*</span> </label>
                <Select<HandicapCode> url="handicaps" code="code_handicap" designation="designation" placeholder="Handicap" onChange={select_handicap} className={`col-span-4 primary ${errors.code_handicap && 'has-error'}`}/>
