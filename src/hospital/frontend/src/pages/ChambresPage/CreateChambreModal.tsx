...
function capArray<T>(data: T[], size: number): T[]{
  if (data.length <= size)
    return [...data, ...Array(size - data.length).fill({type:"", description:""})];
  else
    ...
    <Modal isOpen={isOpen} icon="fa fa-bed" theme={theme} size="sm:max-w-2xl">
      ...
      <form className="grid grid-cols-12 gap-2" onSubmit={handleSubmit(onSubmit)} onReset={onReset}>
        ...
        <div className="flex justify-end gap-3 col-span-12">
            <button type="submit" className={`${ModalThemes[theme].color} rounded-md px-4 py-2 font-semibold text-white`}>Ajouter</button>
