...
function ChambresPage() {
  const [selectedChambre, setSelectedChambre] = useState<Chambre>({num: "", etage: 0, description: "", nombre_lits: undefined, nombre_lits_occupe: undefined });
  ...
  const tableDefinition = useMemo(() => [
      ...
      { header: "Etage", id: "etage", cell: (info) => getEtageName(info.row.original.etage) },
  ...
  return (
    <>
      <Card title="Chambres" className="w-full" action={action}>
        ...
        <CreateChambreModal isOpen={openModal==="create"} close={() => setOpenModal("")}/>
        <ViewChambreModal   isOpen={openModal==="view"} close={() => setOpenModal("")} selectedChambre={selectedChambre} />
        <EditChambreModal   isOpen={openModal==="edit"} close={() => setOpenModal("")} selectedChambre={selectedChambre} />
        <DeleteChambreModal isOpen={openModal==="delete"} close={() => setOpenModal("")} selectedChambre={selectedChambre} />
      </Card>
    </>
  );
}
