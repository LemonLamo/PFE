...
function MesPatientsPage() {
  ...
  const tableDefinition = useMemo(
    () => [
      ...
      {
        header: "Date et lieu de naissance",
        id: "date_lieu_naissance",
        cell: (info) => (
          <>
            {" "}
            {moment(info.row.original.date_naissance).format(
              "DD/MM/YYYY"
            )}, {info.row.original.lieu_naissance}
          </>
        ),
      },
          ...
          return (
            <div className="flex justify-end gap-2">
              ...
              >
                <button
                  onClick={() => null}
                  className="w-4 text-green-500 hover:text-green-700 hover:scale-110"
                >
