...
function Medicaments() {
  ...
  const dictionnaire_medicaments = medicaments.map(f => ({key: f.code, value: f.nom}))
  const [selectedMedicament, setSelectedMedicament] = useState<StockMedicament>({ code: '', nom: '', quantite: 0});
  ...
  function view_medicament(code: string) {
    ...
    setSelectedMedicament(m => ({ ...m, code: code, nom:"Paracétamol", quantite: 5 }))
  }
  ...
  function build_badge(qte: number){
    if(qte >= 10)
      return <Badge bgColor="#dcfce7" textColor="#267142">
        <CheckCircleIcon className="h-[2vh] mr-1" />
        En stock
      </Badge>

    else if (qte > 0)
      return <Badge bgColor="#fdba74" textColor="#9a3412">
        <ExclamationTriangleIcon className="h-[2vh] mr-1" />
        Près de repture
      </Badge>
    
    else 
      return <Badge bgColor="#fee2e2" textColor="#991b1b">
        <ExclamationTriangleIcon className="h-[2vh] mr-1" />
        Repture de stock
      </Badge>
  }
  ...
  const createModal = <div className="flex gap-2">
    <RemplirModal onAdd={update_quantite_medicament} onCancel={() => console.log("Cancelled create")}>
      <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3" id="modal-title">Remplir un médicament</h3>
    ...
    <DepenserModal onSubstract={update_quantite_medicament} onCancel={() => console.log("Cancelled create")}>
      <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3" id="modal-title">Consommer un médicament</h3>
      <p className="text-gray-600">Remplissez ce formulaire pour ajouter une prescription à la consultation courante.</p>
      <div className="mb-2">
        <label className="text-sm font-semibold">Code:</label>
        <Select options={dictionnaire_medicaments} placeholder="Médicament" onChange={select_medicament} state={{ key: selectedMedicament.code, value: selectedMedicament.nom! }} />
  ...
  return (
    <>
      <Card title="Stock Medicaments" action={createModal} className="w-full">
        <Table fields={["Code", "Nom", "Quantité en stock", "Niveau de disponibilité", "", ]}>
           { medicaments.map((m, i) => (
            <TableRow key={i}>
               ...
               <TableCell className="pe-3 py-2"> {build_badge(m.quantite)} </TableCell>
               <TableCell className="flex justify-end gap-2">
                 <ViewModal onOpen={() => view_medicament(m.code)}>
                   <h3 className="text-lg font-semibold leading-6 text-gray-900 mb-3" id="modal-title">Détails sur "{m.nom} ({m.code})"</h3>
                   <p className="text-gray-600">Remplissez ce formulaire pour ajouter une prescription à la consultation courante.</p>
                   <div className="grid grid-cols-12 gap-x-4">
                    ...
                    <Table fields={['#', 'Date', 'Type', 'Avant', 'Après', 'Différence', 'Utilisateur']} className="mb-4 col-span-12 max-h-72">
                      <TableRow>
                 ...
                 <DeleteModal onDelete={() => delete_medicament(m.code)} onCancel={() => console.log("Cancelled delete")}>
