...
function NewPatientPage() {
  ...
  const onSubmit: SubmitHandler<Patient> = async (patient) => {
    const data = {...patient, maladies_chroniques, allergies, antecedents_medicaux, antecedents_familiaux}
    await axios.post(`${baseURL}/api/patients`, data);
  }
  function ajouter_maladie_chronique(maladie_chronique: Maladie){
    setMaladiesChroniques(maladies_chroniques => ([...maladies_chroniques!, maladie_chronique]))
    setOpenModal("")
  }
  ...
  function ajouter_antecedants_medicales(antecedent_medical: Antecedent){
    setAntecedentsMedicaux(antecedents_medicaux => ([...antecedents_medicaux!, antecedent_medical]))
    setOpenModal("")
  }
  ...
  function ajouter_antecedants_familiales(antecedent_familial : Antecedent){
    setAntecedentsFamiliaux(antecedent_familiaux => ([...antecedent_familiaux!, antecedent_familial]))
    setOpenModal("")
  }
      ...
      <form className="grid grid-cols-12 gap-x-4" onSubmit={handleSubmit(onSubmit)}>
        <div className="col-span-12 md:col-span-4">
          ...
          <div className="grid grid-cols-12 gap-x-2">
            ...
            <div className="col-span-6 mb-2">
              ...
              <input type="date" className="primary" {...register('date_de_naissance')} placeholder="Date" />
            </div>
            <div className="col-span-6 mb-2">
              <label className="text-sm font-semibold">Lieu de naissance:</label>
              <input type="text" className="primary" {...register('lieu_de_naissance')} placeholder="ex. Alger" />
            </div>
            <div className="col-span-6 mb-2">
          ...
          <div className="mb-2">
            ...
            <input type="text" className="primary" placeholder="ex. bouguera.med@gmail.com" {...register('email')}/>
          </div>
          <div className="mb-2">
            <label className="text-sm font-semibold">Numéro de téléphone:</label>
            <input type="text" className="primary" placeholder="+213 549297666" {...register('telephone')}/>
          </div>
        </div>
        ...
        <div className="col-span-12 md:col-span-4">
          <div className="mb-0 flex justify-between">
            ...
            <Button type="button" className="h-8"  onClick={() => setOpenModal('ajouter_maladie_chronique')} theme="primary-alternate">
              ...
              maladies_chroniques?.map((x, i)=>(
                <TableRow key={i}>
                  ...
                  <TableCell className="text-right">
                    <DeleteButton onClick={() => {setSelected(i); setOpenModal("delete_maladie_chronique")}} />
                  </TableCell>
                </TableRow>
              ))
            }
          </Table>
          <AjouterMaladieChronique isOpen={openModal==="ajouter_maladie_chronique"} action={ajouter_maladie_chronique} close={() => setOpenModal("")} />
          <DeleteMaladieChroique isOpen={openModal==="delete_maladie_chronique"} action={() => delete_maladie_chronique(selected)} close={() => setOpenModal("")} />

          <div className="mb-0 flex justify-between">
            <h6 className="mb-1">Allergies</h6>
            <Button type="button" className="h-8"  onClick={() => setOpenModal('ajouter_allergie')} theme="primary-alternate">
              <i className="fa fa-plus" />
              <span className="ms-2">Ajouter</span>
            </Button>
          </div>
          <Table fields={['#', 'Nom', 'Date', 'Remarques', '']} className="mb-4">
            {
              allergies?.map((x, i)=>(
                <TableRow key={i}>
                  ...
                  <TableCell>{x.remarques}</TableCell>
                  <TableCell className="text-right">
                    <DeleteButton onClick={() => {setSelected(i); setOpenModal("delete_allergie")}} />
                  </TableCell>
                </TableRow>
              ))
            }
          </Table>
          <AjouterAllergie isOpen={openModal==="ajouter_allergie"} action={ajouter_allergie} close={() => setOpenModal("")} />
          <DeleteAllergie isOpen={openModal==="delete_allergie"} action={() => delete_allergie(selected)} close={() => setOpenModal("")} />

          <div className="mb-0 flex justify-between">
            <h6 className="mb-1">Antécédants Médicales</h6>
            <Button type="button" className="h-8"  onClick={() => setOpenModal('ajouter_antecedants_medicales')} theme="primary-alternate">
              <i className="fa fa-plus" />
              <span className="ms-2">Ajouter</span>
            </Button>
          </div>
          <Table fields={['Designation', 'Date', 'Remarques', '']} className="mb-4">
            {
            antecedents_medicaux?.map((x, i)=>(
              <TableRow key={i}>
                ...
                <TableCell>{x.remarques}</TableCell>
                <TableCell className="text-right">
                  <DeleteButton onClick={() => {setSelected(i); setOpenModal("delete_antecedants_medicales")}} />
                </TableCell>
              </TableRow>
            ))
            }
          </Table>
          <AjouterAntecedentMedical isOpen={openModal==="ajouter_antecedants_medicales"} action={ajouter_antecedants_medicales} close={() => setOpenModal("")} />
          <DeleteAntecedentMedical isOpen={openModal==="delete_antecedants_medicales"} action={() => delete_antecedants_medicales(selected)} close={() => setOpenModal("")} />

          <div className="mb-0 flex justify-between">
            <h6 className="mb-0">Antécédants Familiaux</h6>
            <Button type="button" className="h-8"  onClick={() => setOpenModal('ajouter_antecedants_familiales')} theme="primary-alternate">
              <i className="fa fa-plus" />
              <span className="ms-2">Ajouter</span>
            </Button>
          </div>    
          <Table fields={['Designation', 'Date', 'Remarques', '']} className="mb-4">
            ...
            antecedents_familiaux?.map((x, i)=>(
              <TableRow key={i}>
                ...
                <TableCell>{x.remarques}</TableCell>
                <TableCell className="text-right">
                  <DeleteButton onClick={() => {setSelected(i); setOpenModal("delete_antecedants_familiales")}} />
                </TableCell>
              </TableRow>
            ))
            }
          </Table>
          <AjouterAntecedentFamilial isOpen={openModal==="ajouter_antecedants_familiales"} action={ajouter_antecedants_familiales} close={() => setOpenModal("")} />
          <DeleteAntecedentFamilial isOpen={openModal==="delete_antecedants_familiales"} action={() => delete_antecedants_familiales(selected)} close={() => setOpenModal("")} />
