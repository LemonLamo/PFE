...
async function getHistorique(req, res) {
  ...
  const consultations = (
    await axios.get(
      `http://ehr-service:8080/api/consultations?patient=${NIN}`,
      { headers: { Cookie: req.headers.cookie } }
    )
  ).data;
  const hospitalisations = (
    await axios.get(
      `http://ehr-service:8080/api/hospitalisations?patient=${NIN}`,
      { headers: { Cookie: req.headers.cookie } }
    )
  ).data;
  const interventions = (
    await axios.get(
      `http://ehr-service:8080/api/interventions?patient=${NIN}`,
      { headers: { Cookie: req.headers.cookie } }
    )
  ).data;
  return res
    ...
    .json(
      [...consultations, ...hospitalisations, ...interventions].sort(
        (a, b) =>
          new Date(b.date_consultation ?? b.date_entree ?? b.date) -
          new Date(a.date_consultation ?? a.date_entree ?? a.date)
      )
    );
}
